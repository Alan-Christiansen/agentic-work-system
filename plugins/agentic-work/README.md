...
`start-area` requires a reviewed preflight before writing. It creates an Area Contract v0.2 record for ongoing responsibility, including an explicitly approved agent-role brief and thin folder-local `AGENTS.md` and `CLAUDE.md` bootstrap adapters, keeps Area tasks separate from inactive project candidates, and does not convert or delete an existing project, promote candidates, or perform the represented next action. Its base scaffold is the default; an ongoing software-product venture may receive the optional complete `product-venture` Area pack only when that exact write set is approved.

`start-project` requires a reviewed preflight before writing. It creates a Project Contract v0.3 record with an explicitly approved agent-role brief and thin folder-local `AGENTS.md` and `CLAUDE.md` bootstrap adapters. A `software-product` profile defines one bounded release and links relevant durable product direction from an explicitly supplied stewardship Area rather than copying it; it also works without an Area. For `software-product` and `software-tool`, the skill creates an approved local stub repository or links an existing local repository and synchronizes the role brief into separate repository-local guidance. It does not add a remote, dependency, commit, publication, deployment, or installation change, and it does not begin the represented next action.
