...
`start-area` requires a reviewed preflight before writing. It creates an Area Contract v0.2 record for ongoing responsibility, including an explicitly approved agent-role brief, keeps Area tasks separate from inactive project candidates, and does not convert or delete an existing project, promote candidates, or perform the represented next action.

`start-project` requires a reviewed preflight before writing. It creates a Project Contract v0.3 record with an explicitly approved agent-role brief. For `software-product` and `software-tool`, it creates an approved local stub repository or links an existing local repository and synchronizes the role brief into repository-local guidance. It does not add a remote, dependency, commit, publication, deployment, or installation change, and it does not begin the represented next action.
