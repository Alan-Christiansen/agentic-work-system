...
3. Area- or project-local `AGENTS.md` bootstrap adapter
...
Agentic PM is a reusable capability implemented in the independently versioned `agentic-pm` plugin. Its user-facing management surface has two adaptive entrypoints: `review` understands managed state and decides what deserves attention at the appropriate portfolio, domain, Area, Project, or inactive-candidate scope; `focus` turns an accepted direction into a realistic weekly plan or one finishable session move, diagnosing stuckness when it blocks progress. Detailed modes remain internal references rather than separate commands. Normal PM operation selects a single domain, loads only explicitly managed state within that domain, and sizes recommendations to the time and attention available for that session. Availability is transient input rather than durable Area metadata.
...
Reusable skills are distributed through thin host-specific plugin manifests around provider-neutral skill sources. `agentic-work` owns Area and Project initialization plus checkpointing. The separate `agentic-pm` plugin owns adaptive review, adaptive focus, and deterministic dashboard refresh. Review routes internally among portfolio, Area, Project, and inactive-candidate modes; focus routes between weekly and session horizons and treats stuckness as a diagnostic mode. Keeping both plugins in this repository preserves one protocol while allowing their versions, installation, and sharing to evolve independently.
